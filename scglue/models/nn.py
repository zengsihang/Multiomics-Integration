...
class GraphConv(torch.nn.Module):
    ...
    def forward(
        ...
        message_square = input[sidx] * input[sidx] * (esgn * enorm).unsqueeze(1)
        num_nodes = input.shape[0]
    
        degree = scatter_add(esgn * enorm, tidx, dim_size=num_nodes)
        degree = degree.unsqueeze(-1) + 1
        
        tidx = tidx.unsqueeze(1).expand_as(message)  # n_edges * n_features

        mean = scatter_mean(message, tidx, dim=0, dim_size=num_nodes)
        sq_mean = scatter_mean(message_square, tidx, dim=0, dim_size=num_nodes)
        max = scatter_max(message, tidx, dim=0, dim_size=num_nodes)[0]
        min = scatter_min(message, tidx, dim=0, dim_size=num_nodes)[0]
        ...
        res = (features.unsqueeze(-1) * scales.unsqueeze(-2)).flatten(-2)
